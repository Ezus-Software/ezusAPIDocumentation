...
| contact        | JSON   | A single JSON element ([Contacts](#contacts)) representing the main<br/>Note: In this endpoint, an extra parameter 'mode' is available for the 'contact' object. Write only option. Allowed values are: 'insert_main' (default) & 'upsert_main'. If the value specified for the contact mode is 'insert_main', a new contact (of a client) will added and will be set as the main contact. If the mode is 'upsert_main', if a main contact already exists for the client, it will be updated. Otherwise, a new contact will still be created. |
