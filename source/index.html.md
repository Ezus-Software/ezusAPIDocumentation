...
| contact        | JSON   | Main contact object ([Contacts](#contacts)). Use <code>mode</code> in this endpoint as an extra parameter if needed. Mode values can either be: `insert_main` (default, if not specidief) creates & set a main contact even if exists ; `upsert_main` update the main contact if exists, else create it |
